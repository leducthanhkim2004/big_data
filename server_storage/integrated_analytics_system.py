#!/usr/bin/env python3
"""
Integrated Distributed Data Analytics System
Combines task scheduling, data processing, and monitoring
"""
...
class IntegratedAnalyticsSystem:
    """Complete integrated analytics system"""
    ...
    def start(self):
        ...
        logger.info("Integrated analytics system started")
    ...
    def stop(self):
        ...
        logger.info("Integrated analytics system stopped")
    ...
    def start_tcp_server(self):
        ...
        try:
            ...
            logger.info(f"Integrated TCP server listening on {self.scheduler_host}:{self.scheduler_port}")
        ...
        try:
            ...
            try:
                ...
                if 'task_id' in data and 'data' in data:
                    ...
                    self._handle_data_task(client_id, data)
                    return
    ...
    def _handle_data_task(self, client_id: str, task_data: Dict):
        ...
        try:
            ...
            logger.info(f"Received data task assignment: {task_id} for {file_name} "
                       f"({len(data_block)} bytes)")
            
            # Verify checksum
            calculated_checksum = hashlib.sha256(data_block).hexdigest()
            if calculated_checksum != checksum:
                logger.error(f"Checksum verification failed for task {task_id}")
                return
            
            # Send task data to client
            self._send_task_data_to_client(client_id, task_data)
            
        except Exception as e:
            logger.error(f"Error handling data task: {e}")
    
    def _send_task_data_to_client(self, client_id: str, task_data: Dict):
        """Send task data to client"""
        try:
            with self.client_lock:
                if client_id not in self.clients:
            ...
            self.stats['total_bytes_distributed'] += len(bytes.fromhex(task_data['data']))
            
            logger.info(f"Sent task {task_data['task_id']} to client {client_id} "
                       f"({len(bytes.fromhex(task_data['data']))} bytes)")
    ...
    def _monitoring_loop(self):
        """Monitoring loop for system status"""
        while self.running:
            try:
    ...
    def get_statistics(self) -> Dict:
        ...
        return {
            ...
            'throughput_mbps': (self.stats['total_bytes_distributed'] / runtime / (1024*1024)) if runtime > 0 else 0
        }
    ...
    def print_status(self):
        ...
        print("\n" + "=" * 60)
        print(f"Integrated Analytics System Status - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        print("=" * 60)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Integrated Analytics System')
    parser.add_argument('--host', default='0.0.0.0', help='System host')
    parser.add_argument('--port', type=int, default=9000, help='System port')
    ...
    try:
        ...
        if args.data_dir:
            system.plan_tasks_from_directory(args.data_dir)
        
        # Start system
        system.start()
        
        print("=" * 60)
        print("Integrated Distributed Data Analytics System")
        print("=" * 60)
